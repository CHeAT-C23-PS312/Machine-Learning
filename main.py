...
# Create a dictionary based on it's tag
...
async def predict_label(req: MessagesReq):
    ...
    predicted_recipe = [recipe_name[label] for label in top_labels]
    predicted_ingredients = [ingredients[label] for label in top_labels]
    predicted_serving_size = [serving_size[label] for label in top_labels]
    predicted_servings = [servings[label] for label in top_labels]
    predicted_instruction = [instruction[label] for label in top_labels]
    predicted_calories = [calories[label] for label in top_labels]
    predicted_image_path = [image[label] for label in top_labels]
    
    return {
        "recipe_name": predicted_recipe,
        "ingredients": predicted_ingredients,
        "serving_size": predicted_serving_size,
        "servings": predicted_servings,
        "instruction": predicted_instruction,
